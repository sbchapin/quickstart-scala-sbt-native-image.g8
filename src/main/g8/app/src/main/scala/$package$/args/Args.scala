...
object Args {
  ...
  private val piCommand: Command[Cmd] = Command(
    ...
    val iterationsOpt = Opts
      .option[Long](
        long = "iterations",
        short = "i",
        help = "Number of iterations. The higher this is, the more accurate (and expensive) the calculation becomes."
      )
      .withDefault(Defaults.Pi.iterations)

    val parallelismOpt = Opts
      .option[Int](long = "parallelism", short = "p", help = "Maximum allowed number of parallel calculations.")
  ...
  private val hiCommand: Command[Cmd] = Command(
    ...
    val nameOpt =
      Opts
        .option[String](long = "name", short = "n", help = "Name to generate a greeting for.")
        .withDefault(Defaults.Hi.name)
